...
router.post("/", (req, res) => {
  ...
  if (!name || !description || name.length > 128) {
    ...
    res.status(400).json({
      errorMessage:
        "Please provide name (less than 128 characters) and description for the project."
    });
    return;
  }
  projectModel
...
router.delete("/:id", (req, res) => {
  ...
  projectModel
    ...
    .then(response => {
      ...
      res.json({ message: "Project removed from system!" });
    })
    .catch(error => {
...
router.get("/:id/actions", (req, res) => {
  projectModel
    ...
    .catch(err => {
      res.status(500).json({
        error: "The project's actions coult not be retrieved. Sorry!"
      });
    });
});
